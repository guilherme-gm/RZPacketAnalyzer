...
namespace RZPacketAnalyzer.Utils
{
    ...
    public class SettingsReader
    {
        ...
        private void LoadSettings(string path, int depth)
        {
            ...
            {
                MessageBox.Show(string.Format("Too many recursive calls ({0}) to settings import detected. Loading of {1} aborted.", MaximumDepth, path));
                return;
            }
            else if (!File.Exists(path))
            {
                MessageBox.Show(string.Format("Could not find Settings File at {0}. Skipping it.", path));
                return;
            }
        ...
        public byte ReadByte(string name, byte defaultValue, byte min, byte max)
        {
            ...
            {
                MessageBox.Show(
                    string.Format("Setting '{0}' not found. Using default value ({1}).",
                    name, defaultValue)
                );
                return defaultValue;
            }
            ...
            {
                MessageBox.Show(
                    string.Format("Value of setting '{0}' must be a number, changing back to default ({1}).",
                    name, defaultValue)
                );
                return defaultValue;
            }
            ...
            {
                MessageBox.Show(
                    string.Format("Value of setting '{0}' ({1}) is out of allowed range ({2} - {3}). Changing back to default ({4})",
        ...
        public string ReadString(string name, string defaultValue, bool empty)
        {
            ...
            {
                MessageBox.Show(
                    string.Format("Value of setting '{0}' not be empty, changing back to default ({1}).",
                    name, defaultValue)
                );
                return defaultValue;
            }
        ...
        public object ReadObject(string name, Type type)
        {
            ...
            return JsonConvert.DeserializeObject(entry.ToString(), type);
        }
        ...
        public bool ReadBoolean(string name, bool defaultValue)
        {
            ...
            if (!bool.TryParse(entry.ToString(), out value))
            {
                ...
                {
                    MessageBox.Show(
                        string.Format("Value of setting '{0}' must be true(1) or false(0), changing back to default ({1}).",
                        name, defaultValue)
                    );
                    return defaultValue;
                }
